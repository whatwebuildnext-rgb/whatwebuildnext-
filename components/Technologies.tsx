...
export const Technologies: React.FC = () => {
  ...
  const [skillsview, setskillsview] = useState<string[]>([])


  let viewcategory = async () => {
    const { data, error } = await supabase.from("skills").select("*").eq("status", true).order("id", { ascending: true });
    if (error) {
      console.log(error);
    } else {
      setskillsview(data);
